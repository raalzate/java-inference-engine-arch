...
public class ClusteringAlgorithm {
    ...
    public List<Cluster> createClusters(DependencyGraph dependencyGraph) {
        ...
        if (isSingleDomain) {
            ...
            // If clusters mix domains, or clustering doesn't work well, fallback to domain-based
            if (hasMultiDomainClusters || clusters.size() < 2 || hasLargeSingleCluster(clusters, dependencyGraph.getComponents().size())) {
                clusters = createDomainBasedClusters(dependencyGraph, componentsByDomain);
            }
            
            // If we still get too few clusters, try to split based on JPA entities and dependencies
            if (clusters.size() < 2) {
                clusters = createEntityBasedClusters(dependencyGraph, componentsByDomain);
            }
        }
    ...
    private boolean isSingleDomainProject(Map<String, List<Component>> componentsByDomain, int totalComponents) {
        if (componentsByDomain.isEmpty()) return false;
     ...
     */
    private List<Cluster> createDomainBasedClusters(DependencyGraph dependencyGraph, Map<String, List<Component>> componentsByDomain) {
        List<Cluster> clusters = new ArrayList<>();
        int clusterId = 0;
        ...
        for (Component component : dependencyGraph.getComponents()) {
            if (!usedComponents.contains(component.getId())) {
                if (!clusters.isEmpty()) {
                    ...
                    int bestClusterIndex = findBestClusterForComponent(component, clusters, dependencyGraph.getComponents());
     ...
     */
    private List<Cluster> createEntityBasedClusters(DependencyGraph dependencyGraph, Map<String, List<Component>> componentsByDomain) {
        List<Cluster> clusters = new ArrayList<>();
        ...
        for (Component component : dependencyGraph.getComponents()) {
            if (assignedComponents.contains(component.getId())) continue;
     ...
     */
    private List<Cluster> createBusinessResponsibilityClusters(DependencyGraph dependencyGraph, Map<String, List<Component>> componentsByDomain) {
        List<Cluster> clusters = new ArrayList<>();
        ...
        for (Map.Entry<String, List<Component>> domainEntry : componentsByDomain.entrySet()) {
            ...
            if (domain.equals("core")) continue;
            ...
            // If domain has multiple functions, keep them separate; if only one, group all together
        ...
        // CONSOLIDATE: Merge singleton entity/port/DTO clusters into their domain's main cluster
    ...
    /**
     * Identifies business functions from component names (Controller, Service patterns).
     */
     ...
     * ONLY considers role-bearing components (Service, Controller, Repository, UseCase, etc.).
     * Returns null for entities, DTOs, events, and domain objects to avoid over-fragmentation.
     */
    private String extractBusinessFunction(String className) {
        ...
        // IGNORE simple domain objects in domain/ports packages (likely Lombok entities)
    ...
    private String findBestBusinessFunctionMatch(String componentName, Set<String> existingComponents) {
        ...
        for (String existingComponent : existingComponents) {
            ...
            if (function != null) {
                // Check if the simple component name contains the function name as a word boundary
    ...
    private boolean hasLargeSingleCluster(List<Cluster> clusters, int totalComponents) {
        ...
        // Lowered threshold: If one cluster has more than 50% of components, consider it too large
     ...
     */
    private int findBestClusterForComponent(Component component, List<Cluster> clusters, List<Component> allComponents) {
    ...
    private boolean isJpaEntity(Component component) {
        ...
        return className.contains("entity") || 
               ...
               (!component.getTablesUsed().isEmpty() && !className.contains("repository") && !className.contains("service"));
    }
    ...
    private boolean isRelatedToEntity(String componentName, String entityName) {
        ...
        // Check if class name starts with entity name (e.g., AirportService, AirportRepository, AirportController)
        if (simpleClassName.startsWith(lowerEntityName)) {
            return true;
        }
        
        // Check if class name contains entity name (e.g., AirportServiceImpl, DefaultAirportService)
     ...
     * Skips organizational prefixes and framework names to find meaningful domain segments.
     */
    private String inferDomainFromComponentId(String componentId) {
        ...
        while (startIndex < parts.length && parts[startIndex].matches("(spring|boot|jakarta|javax|hibernate|jpa|monolith)")) {
            startIndex++;
        }
        ...
        if (startIndex < parts.length) {
            ...
            if (domainCandidate.matches("(main|app|application|common|config|configuration|dto|api|rest|web)")) {
                ...
                if (startIndex < parts.length) {
                    ...
                    if (!domainCandidate.matches("(service|services|util|utils|helper|helpers|model|models|entity|entities|controller|controllers|repository|repositories|dao)")) {
     ...
     */
    private List<Cluster> createDomainClusters(List<Component> domainComponents, int startClusterId, DependencyGraph graph) {
        List<Cluster> clusters = new ArrayList<>();
        
        // Strategy: Group components that share tables or have strong call relationships
    ...
    /**
     * Consolidates singleton clusters by merging entities/DTOs/ports into their domain's main cluster.
     * This prevents over-fragmentation from Lombok entities and simple domain objects.
     */
    private List<Cluster> consolidateSingletonClusters(List<Cluster> clusters, List<Component> allComponents) {
        ...
        // For each domain, consolidate singleton entity/DTO/port clusters into the largest cluster
    ...
    /**
     * Determines if a component should be merged into the main cluster of its domain.
     * Returns true for entities, DTOs, events, ports, and other non-role-bearing components.
     */
    private boolean shouldMergeIntoMainCluster(Component component) {
        ...
        if ((packagePath.contains(".domain.") || packagePath.contains(".primaryports.") || 
            ...
            if (!simpleName.matches(".*(Service|UseCase|Repository|Repo|Db|Publisher|Factory|Handler|Operations?|Listener|Adapter|Controller)$")) {
